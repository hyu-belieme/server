...
@Table(name = "HISTORY_", uniqueConstraints = {
    @UniqueConstraint(name = "history_index", columnNames = {"item_id", "num"})})
@NoArgsConstructor
@ToString
@Getter
public class HistoryEntity extends DataEntity<UUID> {
  ...
  public HistoryEntity(@NonNull UUID id, @NonNull ItemEntity item, int num, UserEntity requester,
      UserEntity approveManager, UserEntity returnManager, UserEntity lostManager,
      UserEntity cancelManager, long requestedAt, long approvedAt, long returnedAt, long lostAt,
      long canceledAt) {
    this.id = id;
  ...
  public HistoryEntity withItem(@NonNull ItemEntity item) {
    return new HistoryEntity(id, item, num, requester, approveManager, returnManager, lostManager,
  ...
  public HistoryDto toHistoryDto() {
    return new HistoryDto(id, item.toItemDto(), num, getUserDtoOrNull(requester),
        getUserDtoOrNull(approveManager), getUserDtoOrNull(returnManager),
        getUserDtoOrNull(lostManager), getUserDtoOrNull(cancelManager), requestedAt, approvedAt,
        returnedAt, lostAt, canceledAt);
  }

  public HistoryDto toHistoryDtoNestedToItem() {
    return new HistoryDto(id, ItemDto.nestedEndpoint, num, getUserDtoOrNull(requester),
        getUserDtoOrNull(approveManager), getUserDtoOrNull(returnManager),
        getUserDtoOrNull(lostManager), getUserDtoOrNull(cancelManager), requestedAt, approvedAt,
        returnedAt, lostAt, canceledAt);
  }
