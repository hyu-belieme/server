...
public class MajorDaoTest extends BaseDaoTest {
  ...
  public class TestGetByIndex {
    ...
    public void SUCCESS() {
      ...
      when(majorRepository.findByUniversityIdAndCode(univId, majorCode)).thenReturn(
          Optional.of(major));
      TestHelper.objectCompareTest(this::execMethod, major.toMajorDto());
    }
  ...
  public class TestCreate {
    ...
    public void SUCCESS() {
      ...
      when(univRepository.findById(major.getUniversityId())).thenReturn(
          Optional.of(major.getUniversity()));
      when(majorRepository.existsById(major.getId())).thenReturn(false);
      when(majorRepository.existsByUniversityIdAndCode(major.getUniversityId(),
          major.getCode())).thenReturn(false);
  ...
  public class TestUpdate {
    ...
    private MajorEntity updatedMajor() {
      return target.withUniversity(newMajor.getUniversity()).withCode(newMajor.getCode());
    }
    ...
    public void SUCCESS() {
      ...
      newMajor = randomMajor().withUniversity(randomUniv()).withCode("__NEW_CODE");

      when(majorRepository.findById(targetId)).thenReturn(Optional.of(target));
      when(univRepository.findById(newMajor.getUniversityId())).thenReturn(
          Optional.of(newMajor.getUniversity()));
      when(majorRepository.existsByUniversityIdAndCode(newMajor.getUniversityId(),
          newMajor.getCode())).thenReturn(false);
    ...
    public void ERROR_IndexConflict_ConflictException() {
      ...
      when(univRepository.findById(newMajor.getUniversityId())).thenReturn(
          Optional.of(newMajor.getUniversity()));
      when(majorRepository.existsByUniversityIdAndCode(newMajor.getUniversityId(),
          newMajor.getCode())).thenReturn(true);
