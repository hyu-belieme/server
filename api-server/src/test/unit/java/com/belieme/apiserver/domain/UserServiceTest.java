...
import com.belieme.apiserver.error.exception.BadGatewayException;
...
public class UserServiceTest extends BaseServiceTest {
  ...
  public final class TestUpdateAuthorityOfUser extends UserNestedTest {
    ...
    public void SUCCESS_requesterIsMasterAndUpdateAuth() {
      setUpDefault();
      setTargetUser(
          randomUserHaveAdditionalAuthButLessPermissionOnDept(authDept, Permission.MASTER));
      setAuthPermission(randomUnderMasterPermission());
    ...
    public void SUCCESS_requesterIsDeveloperAndUpdateAuth() {
      ...
      setRequester(randomDevUser());
      setTargetUser(
          randomUserHaveAdditionalAuthButLessPermissionOnDept(authDept, Permission.DEVELOPER));
    ...
    private UserDto randomUserHaveAdditionalAuthButLessPermissionOnDept(DepartmentDto dept,
  ...
  public final class TestReloadDeveloperUser {
    ...
    private UniversityDto getUnivByUnivIdFromStub(UUID univId) {
      ...
      return new UniversityDto(univInfo.id(), univInfo.name(),
          univInfo.externalApiInfo().get("url"));
    }
    ...
    public void SUCCESS_userIsAlreadyOnDatabase() {
      ...
      verify(userDao).update(eq(userInfo.id()), eq(univId), eq(userInfo.studentId()),
          eq(userInfo.name()), eq(userInfo.entranceYear()), any(), eq(targetUser.createdAt()),
          anyLong(), authListCaptor.capture());
      List<AuthorityDto> authDtoList = authListCaptor.getValue();
      Assertions.assertThat(checkMatchAuthDtoAndInfoList(authDtoList, userInfo.authorities()))
          .isTrue();
    }
    ...
    public void SUCCESS_devIsNotOnDatabase() {
      ...
      verify(userDao).create(any(), eq(univId), eq(userInfo.studentId()), eq(userInfo.name()),
          eq(userInfo.entranceYear()), any(), anyLong(), anyLong(), authListCaptor.capture());
      List<AuthorityDto> authDtoList = authListCaptor.getValue();
      Assertions.assertThat(checkMatchAuthDtoAndInfoList(authDtoList, userInfo.authorities()))
          .isTrue();
    }
  ...
  public final class TestUpdateUserFromHanyangUniversity {
    ...
    public void SUCCESS_userIsAlreadyCreatedAndNoMajorCreate() {
      ...
      when(HttpRequest.getUserInfoFromHanyangApi(apiUrl, clientKey, apiToken)).thenReturn(
          makeJsonResponse());
    ...
    public void ERROR_networkProblemOnHanyangApi_BadGateWayException() {
      ...
      when(initialData.universityInfos()).thenReturn(stub.INIT_DATA.universityInfos());
      when(HttpRequest.getUserInfoFromHanyangApi(apiUrl, clientKey, apiToken)).thenThrow(
  ...
  private RandomGetter<UserDto> usersHaveAdditionalAuthOnDept(RandomGetter<UserDto> rs,
      DepartmentDto dept) {
  ...
  private RandomGetter<UserDto> usersNotHaveAdditionalAuthOnDept(RandomGetter<UserDto> rs,
      DepartmentDto dept) {
    return rs.filter((user) -> {
      for (AuthorityDto auth : user.authorities()) {
        if (auth.department().matchId(dept) && auth.permission() != Permission.DEFAULT) {
