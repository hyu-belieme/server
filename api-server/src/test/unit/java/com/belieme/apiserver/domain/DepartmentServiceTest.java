...
public class DepartmentServiceTest extends BaseServiceTest {
  ...
  public class TestCreate extends DeptNestedTest {
    ...
    public void SUCCESS_noNewMajorCode() {
      ...
      when(userDao.getByToken(userToken)).thenReturn(requester);
      when(departmentDao.create(any(), eq(univId), eq(deptName), eq(baseMajorIds))).thenReturn(
          dept);
    ...
    public void ERROR_getInvalidIndex_InvalidIndexException() {
      ...
      when(userDao.getByToken(userToken)).thenReturn(requester);
      when(departmentDao.create(any(), eq(univId), eq(deptName), eq(baseMajorIds))).thenThrow(
  ...
  public class TestUpdate extends DeptNestedTest {
    ...
    @Override
    protected DepartmentDto execMethod() {
      return departmentService.update(userToken, targetDeptId, newName, newBaseMajorCodes);
    }
    ...
    public void SUCCESS_someMemberIsNull() {
      ...
      verify(departmentDao).update(targetDeptId, targetDeptUnivId, newName,
          toMajorIds(targetDept.baseMajors()));
      verify(majorDao, never()).create(any(), any(), any());
    }
    ...
    public void departmentConflict_ConflictException() {
      ...
      when(
          departmentDao.update(targetDeptId, targetDeptUnivId, newName, newBaseMajorIds)).thenThrow(
          ConflictException.class);
  ...
  private abstract class DeptNestedTest extends BaseNestedTest {
    ...
    protected void mockGetMajors(UUID univId, List<String> majorCodes, List<MajorDto> majors) {
      for (int i = 0; i < majorCodes.size(); i++) {
        ...
        when(majorDao.getByIndex(univId, majorCode)).thenReturn(majors.get(i));
      }
    }

    protected void mockGetOrCreateMajors(UUID univId, List<String> majorCodes,
        List<MajorDto> majors) {
      for (int i = 0; i < majorCodes.size(); i++) {
        String majorCode = majorCodes.get(i);
        if (i == 0) {
          when(majorDao.getByIndex(univId, majorCode)).thenThrow(NotFoundException.class);
          when(majorDao.create(any(), eq(univId), eq(majorCode))).thenReturn(majors.get(i));
          continue;
        }
        when(majorDao.getByIndex(univId, majorCode)).thenReturn(majors.get(i));
      }
